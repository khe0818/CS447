...
object fp3 {
  ...
  def allEqual (xs : List[Int]) : Boolean = {
    // TODO: Provide definition here.
    def f (xs: List[Int], y: Boolean): Boolean = {
      (xs,y) match {
        case (Nil, true) => true 
        case (Nil, false) =>  false
        case (x::Nil, false) => false
        case (x::Nil, true) => true
        case (x::xs, false) =>false 
        case (x::xs, true) => if(x == xs.head ){
          f(xs,true)
        } 
        else{
          f(xs,false)
  ...
  def removeDupes2 [X] (xs:List[X]) : List[(Int, X)] = {
    // TODO: Provide definition here.
    def f [X] (count:Int, ys:List[X]):List[(Int,X)] ={
        ys match {
          case Nil => Nil

...
object fp2 {
  ...
  def append [A] (xs:List[A], ys:List[A]) : List[A] = {
    ...
    (xs, ys) match {
    	case (Nil, Nil) => Nil
    	case (x::xs,Nil) => x::append(xs, Nil)
    	case (Nil, y::ys) =>y::append(Nil, ys)
  ...
  def foldLeft [A,B] (xs:List[A], e:B, f:(B,A)=>B) : B = {
    ...
    (xs,e) match {
      ...
      case (Nil, e) => e
      case (xs, Nil) => Nil
      case (x::xs, e) => foldLeft(xs, f(e,x), f) 
    }
  }
  ...
  def foldRight [A,B] (xs:List[A], e:B, f:(A,B)=>B) : B = {
    ...
    (xs,e) match {
      ...
      case (Nil, e) => e
      case (xs, Nil) => Nil
      case (x::xs, e) => f(x, foldRight(xs, e, f) )
    }
  }
  ...
  def firstNumGreaterThan (a : Int, xs : List[Int]) : Int = {
    // TODO: Provide definition here.
    (a, xs) match {
      case (a, Nil) => throw new java.util.NoSuchElementException 
      case (a, x::xs) => if (x >=a) {
        x
      }
      else{
  ...
  def firstIndexNumGreaterThan (a : Int, xs : List[Int]) : Int = {
    def f (a : Int, xs: List[Int], index : Int) : Int = {
      (a, xs) match {
        case  (a, Nil)=> throw new java.util.NoSuchElementException 
        case (a, x::xs) => if(x >= a){
          index
